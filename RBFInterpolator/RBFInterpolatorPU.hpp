/**
 * RBFInterpolatorPUPU.h
 *  Created on: \date Nov 18, 2010
 *      Author: \author mrehberg
 */

#ifndef RBFInterpolatorPU_H_
#define RBFInterpolatorPU_H_
...
/** \brief Lagrange RBF interpolator implementing a partition of unity method for domain space
 * separation
...
class RBFInterpolatorPU: virtual public NDInterpolator {
...
public:
	...
	RBFInterpolatorPU(
			...
			const ublas::vector<value_type>& data, const value_type scale = 1.,
			const unsigned K = 100, const value_type alpha = .33);
...
private:
	...
	boost::multi_array<RBFInterpolator<RTYPE> , inDimT> interpol;
...
template<class RTYPE, std::size_t inDimT>
RBFInterpolatorPU<RTYPE, inDimT>::RBFInterpolatorPU() {
	inDim = 0;
	numOfPoints = 0;
}
...
RBFInterpolatorPU<RTYPE, inDimT>::RBFInterpolatorPU(
	...
	if (data.size() != numOfPoints)
		throw std::length_error(
				"RBFInterpolatorPU::Constructor: Number of interpolation points (grid.size2()) and number of data points (data.size()) do not match.");
...
template<class RTYPE, std::size_t inDimT>
inline typename RTYPE::value_type RBFInterpolatorPU<RTYPE, inDimT>::eval(
...
template<class RTYPE, std::size_t inDimT>
inline typename RTYPE::value_type RBFInterpolatorPU<RTYPE, inDimT>::evalDiff(
...
template<class RTYPE, std::size_t inDimT>
inline typename RTYPE::value_type RBFInterpolatorPU<RTYPE, inDimT>::evalDiff2(
		const ublas::vector<typename RTYPE::value_type>& inPoint,
		const unsigned alphaDiff) const {
...
template<class RTYPE, std::size_t inDimT>
inline typename RTYPE::value_type RBFInterpolatorPU<RTYPE, inDimT>::evalDiffMixed(
		const ublas::vector<typename RTYPE::value_type>& inPoint,
		const unsigned alpha1, const unsigned alpha2) const {
...
template<class RTYPE, std::size_t inDimT>
inline ublas::vector<typename RTYPE::value_type> RBFInterpolatorPU<RTYPE,
		inDimT>::evalGrad(
		const ublas::vector<typename RTYPE::value_type>& inPoint) const {
...
template<class RTYPE, std::size_t inDimT>
inline ublas::matrix<typename RTYPE::value_type> RBFInterpolatorPU<RTYPE,
		inDimT>::evalHess(
		const ublas::vector<typename RTYPE::value_type>& inPoint) const {
...
template<class RTYPE, std::size_t inDimT>
void RBFInterpolatorPU<RTYPE, inDimT>::init(
	...
	for (unsigned i = 0; i < inDim; i++) {
		...
		// sometimes rounding errors lead to o(i)<0 correct
		if (o(i)<0)
		  o(i)=0;
	...
	for (unsigned i = 0; i < std::pow(M, inDimT); i++) {
		...
		interpol(ind2sub(i)) = RBFInterpolator<RTYPE> (
 ...
 */
template<class RTYPE, std::size_t inDimT>
inline std::vector<ublas::vector<unsigned> > RBFInterpolatorPU<RTYPE, inDimT>::getIndex(
		const ublas::vector<typename RTYPE::value_type>& inPoint) const {
 ...
 */
template<class RTYPE, std::size_t inDimT>
inline void RBFInterpolatorPU<RTYPE, inDimT>::combine(
		ublas::vector<unsigned>& ind,
		std::vector<ublas::vector<unsigned> >& Ind,
 ...
 */
template<class RTYPE, std::size_t inDimT>
inline ublas::vector<unsigned, ublas::bounded_array<unsigned, inDimT> > RBFInterpolatorPU<
